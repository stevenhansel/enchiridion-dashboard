...
const ListFloorPageAPI = (props: Props) => {
  ...
  const handleListFloor = useCallback(async (): Promise<void> => {
    try {
      setIsLoading(true);
      const response = await axios.get<FloorPage>("/v1/floors");
      const floorsResponse: Floor[] = response.data.contents.map(
        (data: Floor) => ({
          id: data.id,
          name: data.name,
...
export default ListFloorPageAPI;
